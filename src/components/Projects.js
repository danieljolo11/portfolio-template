    ...
    >
      ...
      >
        <div className="flex flex-row items-center gap-4 md:gap-10 xl:gap-10 px-2 md:px-4 xl:px-0">
          <span className="text-4xl md:text-6xl xl:text-6xl font-bold text-main-white">Projects</span>
          <div className="w-full h-0.5 bg-main-blue"></div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 place-items-center md:place-items-center xl:place-items-start gap-6 md:gap-4 lg:gap-6">
          {projectsData.map(({ title, techs, desc, img, link }) => {
            const seperatetechs = techs?.join(", ");
            return (
              <div
                className="group flex flex-col h-[15.5rem] md:h-72 lg:h-80 w-auto max-w-[17rem] md:max-w-[19rem] xl:max-w-sm bg-red-600"
                key={title}
              >
                ...
                <div className="flex flex-row basis-1/2">
                  <div className="basis-1/2 bg-cover">
                    <img
                      className="transition-transform origin-bottom-left scale-y-[2.21] md:scale-y-[2] lg:scale-y-[2] scale-x-[2] group-hover:scale-100 duration-300 object-fill object-center h-full w-full"
                      src={img}
                      alt="sampleproject"
                    />
                  </div>
